...
1. Format the request and use `curl` to connect to the `/local` endpoint on the `development`, `testnet06` or `mainnet01` network.
...
This example calls the `greet` function defined in the `simplemodule` that's been deployed in the `free` namespace on the `testnet06` network.
